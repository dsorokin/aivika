...
increase :: MonadSim m => PriorityQueue m a -> m ()
{-# INLINABLE increase #-}
increase pq = 
...
queueNull :: MonadSim m => PriorityQueue m a -> m Bool
{-# INLINABLE queueNull #-}
queueNull pq =
...
queueCount :: MonadSim m => PriorityQueue m a -> m Int
{-# INLINABLE queueCount #-}
queueCount pq = readProtoRef (pqSize pq)

-- | Create a new priority queue.
newQueue :: MonadSim m => Session m -> m (PriorityQueue m a)
{-# INLINABLE newQueue #-}
newQueue session =
...
enqueue :: MonadSim m => PriorityQueue m a -> Double -> a -> m ()
{-# INLINABLE enqueue #-}
enqueue pq k v =
...
dequeue :: MonadSim m => PriorityQueue m a -> m ()
{-# INLINABLE dequeue #-}
dequeue pq =
  do size <- readProtoRef (pqSize pq)
...
queueFront :: MonadSim m => PriorityQueue m a -> m (Double, a)
